...
public class PluginLoggingDependenciesInjector implements ClassRealmManagerDelegate {
    ...
    public void setupRealm(ClassRealm classRealm, ClassRealmRequest request) {
        final ListIterator<ClassRealmConstituent> iter =
                request.getConstituents().listIterator();
        while (iter.hasNext()) {
            final ClassRealmConstituent entry = iter.next();
            // logback
            if ("ch.qos.logback".equals(entry.getGroupId()) && "logback-core".equals(entry.getArtifactId())) {
                iter.remove();
                request.getForeignImports().put("ch.qos.logback", getClass().getClassLoader());
            } else if ("ch.qos.logback".equals(entry.getGroupId()) && "logback-classic".equals(entry.getArtifactId())) {
                iter.remove();
                request.getForeignImports().put("ch.qos.logback", getClass().getClassLoader());
            }
            // jul->slf4j bridge
            else if ("org.slf4j".equals(entry.getGroupId()) && "jul-to-slf4j".equals(entry.getArtifactId())) {
                iter.remove();
            }
            // slf4j->bridge bridge
            else if ("org.slf4j".equals(entry.getGroupId()) && "slf4j-jdk14".equals(entry.getArtifactId())) {
                iter.remove();
            }
            // slf4j-nop bridge
            else if ("org.slf4j".equals(entry.getGroupId()) && "slf4j-nop".equals(entry.getArtifactId())) {
                iter.remove();
            }
            // slf4j-simple bridge
            else if ("org.slf4j".equals(entry.getGroupId()) && "slf4j-simple".equals(entry.getArtifactId())) {
                iter.remove();
            }
            // log4j 1.x
            ...
            // commons-logging
                ...
                request.getForeignImports()
                        .put("org.apache.commons.logging", getClass().getClassLoader());
            }
        }
    }
    ...
    private boolean isLog4j1x(ClassRealmConstituent entry) {
        ...
        if ("org.slf4j".equals(entry.getGroupId()) && "log4j-over-slf4j".equals(entry.getArtifactId())) {
            return true;
        }
        return false;
    }
    ...
    private boolean isCommonsLogging(ClassRealmConstituent entry) {
        if ("commons-logging".equals(entry.getGroupId()) && "commons-logging".equals(entry.getArtifactId())) {
            return true;
        }
